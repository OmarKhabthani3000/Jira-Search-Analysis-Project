...
public class FacetCollector extends Collector {
	...
	private List<Facet> createSortedFacetList(FacetCounter counter, FacetingRequestImpl request) {
		if(FacetSortOrder.RANGE_DEFINITION_ODER.equals(request.getSort())) {
			return createSortedRangeFacetList(counter, request);
		}
		
		List<Facet> facetList = newArrayList();
		List<Map.Entry<String, Integer>> entryList = newArrayList();
		Iterator<Entry<String, Integer>> it = counter.getCounts().entrySet().iterator();
		int includedFacetCount = 0;
		
		while(it.hasNext())
			entryList.add(it.next());
		
		Collections.sort(entryList, new FacetEntryComparator(request.getSort()));
	
		for ( Map.Entry<String, Integer> countEntry : entryList ) {
	...
	private List<Facet> createSortedRangeFacetList(FacetCounter counter, FacetingRequestImpl request) {
		List<Facet> facetList = newArrayList();
		int includedFacetCount = 0;
