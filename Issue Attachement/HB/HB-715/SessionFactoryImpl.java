//$Id: SessionFactoryImpl.java,v 1.26.2.32 2003/12/13 12:25:11 oneovthafew Exp $
...
public final class SessionFactoryImpl implements SessionFactory, SessionFactoryImplementor {
	...
	public SessionFactoryImpl(Configuration cfg, Settings settings) throws HibernateException {
		...
		if ( settings.isAutoUpdateSchema() ) new SchemaUpdate(cfg).execute(false);
	...
	public void evict(Class persistentClass, Serializable id) throws HibernateException {
		...
		if ( p.hasCache() ) p.getCache().remove(id);
	}
	...
	public QueryCache getQueryCache(String cacheRegion) throws
		HibernateException {
		if (cacheRegion==null) return getQueryCache();

		QueryCache currentQueryCache = (QueryCache) queryCaches.get(cacheRegion);
		if (currentQueryCache==null) {
			currentQueryCache = new QueryCache(
	...
	public void evictQueries(String cacheRegion) throws HibernateException {
		if (cacheRegion==null) {
			...
			QueryCache currentQueryCache = (QueryCache) queryCaches.get(cacheRegion);
			if (queryCache!=null) currentQueryCache.clear();
		}
	}	
