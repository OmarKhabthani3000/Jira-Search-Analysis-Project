//$Id: Binder.java,v 1.26.2.21 2003/09/13 16:42:24 oneovthafew Exp $
...
import net.sf.hibernate.mapping.CollectionValue;
...
final class Binder {
	...
	public static void bindJoinedSubclass(Element node, Subclass model, Mappings mappings) throws MappingException {
		...
		model.setKey(key);
		bindValue( keyNode, key, false, model.getName() );

		model.getKey().setType( model.getIdentifier().getType() );
		model.createPrimaryKey();
		model.getKey().createForeignKeyOfClass( model.getSuperclass().getPersistentClass() );
			...
			if ( "id".equals(name) ) {
				...
				if (propertyName==null) {
					...
					if (id.getType()==null) throw new MappingException(
						"must specify an identifier type: " + model.getPersistentClass().getName()
					);
				...
				if (propertyName==null) {
					...
					Class reflectedClass = ReflectHelper.reflectedPropertyClass( model.getPersistentClass(), propertyName );
					bindComponent(
						subnode,
						id,
						reflectedClass,
						model.getName() + StringHelper.DOT + propertyName,
						false,
						mappings
					);
					Property prop = new Property(id);
				...
				String cname = model.getPersistentClass().getName();
				CacheConcurrencyStrategy cache = createCache(subnode, cname, model);
				mappings.addCache(cname, cache);
	...
	public static void bindMetaValues(Element node, Value model) {
		//META(S)
		Iterator iter = node.elementIterator("meta");
		while( iter.hasNext() ) {
			Element subnode = (Element) iter.next();
			String key = subnode.attributeValue("attribute");
			String value = subnode.getText();
			model.getMetaValues().put(key, value);
		}
	}

	public static void bindColumns(Element node, Value model, boolean isNullable, boolean autoColumn, String defaultColumnName) {
		//COLUMN(S)
		...
		if ( columnNode==null) {
			...
			while( iter.hasNext() ) {
				...
				col.setName( subnode.attributeValue("name") );
				...
				Attribute uniqueNode = subnode.attribute("unique-key");
	...
	public static void bindProperty(Element node, Property model, Mappings mappings) throws MappingException {
		...
		model.setName( getPropertyName(node) );
		...
		if (accessNode!=null) model.setPropertyAccessorName( accessNode.getValue() );
		...
		model.setLength( (lengthNode==null) ?
			-1:
			Integer.parseInt( lengthNode.getValue() )
		);
	...
	public static void bindCollection(Element node, Collection model, String prefix, Mappings mappings) throws MappingException {
		...
		model.setRole( prefix + StringHelper.DOT + propertyName );
		...
		if (oneToManyNode!=null) {
			...
			bindOneToMany( oneToManyNode, model.getOneToMany() );
			...
			log.info("Mapping collection: " + model.getRole() + " -> " + model.getTable().getName() );
		}
		//LAZINESS
		...
		if (typeNode!=null) {
			try {
				model.setType( TypeFactory.manyToOne( 
					ReflectHelper.classForName( typeNode.getValue() ), 
	...
	public static void bindOneToOne(Element node, OneToOne model, boolean isNullable, Mappings mappings) throws MappingException {
		...
		model.setForeignKeyType(
			constrained ?
				AssociationType.FOREIGN_KEY_FROM_PARENT :
				AssociationType.FOREIGN_KEY_TO_PARENT
		);
	...
	public static void bindOneToMany(Element node, OneToMany model) throws MappingException {
		try {
			model.setType( (EntityType) Hibernate.entity(
				ReflectHelper.classForName( node.attributeValue("class") )
			) );
		}
		catch (ClassNotFoundException cnfe) {
				...
				if ( "element".equals(name) ) {
					...
					try {
						model.setElementClass(
							ReflectHelper.classForName( subnode.attributeValue("class") )
						);
					}
					catch (ClassNotFoundException cnfe) {
						throw new MappingException(cnfe);
	...
	public static void bindComponent(Element node, Component model, Class reflectedClass, String path, boolean isNullable, Mappings mappings) throws MappingException {
		...
		if (dynaclassNode!=null) {
			...
			model.setComponentClass(
				model.getOwner().getPersistentClass()
			);
		...
		while ( iter.hasNext() ) {
			...
			if (collectType!=null) {
				...
				value = new CollectionValue( model.getTable(), collection );
			}
				...
				if ( value.getFormula()!=null ) throw new MappingException("properties of components do not support formula: " + propertyName);
				bindValue(subnode, value, isNullable, propertyName);
			}
			else if (
				"component".equals(name) ||
				"dynabean".equals(name) ||
		...
		String[] accessors = new String[span];
		...
		while ( iter.hasNext() ) {
			...
			accessors[i] = prop.getPropertyAccessorName();
			types[i] = prop.getType();
			cascade[i] = prop.getCascadeStyle();
			joinedFetch[i] = prop.getValue().getOuterJoinFetchSetting();
			if ( model.getComponentClass()==null ) dynaprops[i] = new DynaProperty( names[i], types[i].getReturnedClass() );
			i++;
		}
		if ( model.getComponentClass()==null ) model.setDynaClass(
			new BasicDynaClass(className, BasicDynaBean.class, dynaprops)
		);

		model.setType(
			( model.getComponentClass()==null ) ?
				(Type) new DynaBeanType(model.getDynaClass(), names, types, joinedFetch, cascade) :
	...
	private static void makeIdentifier(Element node, Value model, Mappings mappings) {
		//GENERATOR
	...
	protected static void propertiesFromXML(Element node, PersistentClass model, Mappings mappings) throws MappingException {
		...
		while( iter.hasNext() ) {
			...
			if ( value!=null) model.addProperty( createProperty(value, propertyName, model.getPersistentClass(), subnode, mappings) );
		}
	}
	...
	throws MappingException {
		...
		if (parentClass!=null) value.setTypeByReflection(parentClass, propertyName);
		
		if ( value instanceof Association ) { //this is done here 'cos we might only know the type here (ugly!)
			String propertyRef = ( (Association) value ).getReferencedPropertyName();
	...
	public static void bindListSecondPass(Element node, List model, java.util.Map classes, Mappings mappings) throws MappingException {
		...
		bindIntegerValue( subnode, iv, IndexedCollection.DEFAULT_INDEX_COLUMN_NAME, model.isOneToMany() );
		model.setIndex(iv);
	...
	public static void bindIdentifierCollectionSecondPass(Element node, IdentifierCollection model, java.util.Map persistentClasses, Mappings mappings) throws MappingException {
		...
		bindValue(subnode, id, false, IdentifierCollection.DEFAULT_IDENTIFIER_COLUMN_NAME);
		model.setIdentifier(id);
		makeIdentifier(subnode, id, mappings);
		if ( !model.isOneToMany() ) model.createPrimaryKey();
	...
	public static void bindMapSecondPass(Element node, Map model, java.util.Map classes, Mappings mappings) throws MappingException {
		...
		while( iter.hasNext() ) {
			...
			if ( "index".equals(name) ) {
				...
				bindValue(subnode, value, model.isOneToMany(), IndexedCollection.DEFAULT_INDEX_COLUMN_NAME);
				model.setIndex(value);
				if ( model.getIndex().getType()==null ) throw new MappingException("map index element must specify a type");
			}
	...
	public static void bindCollectionSecondPass(Element node, Collection model, java.util.Map persistentClasses, Mappings mappings) throws MappingException {
		...
		if ( model.isOneToMany() ) {
			Class assocClass = model.getOneToMany().getType().getAssociatedClass();
		...
		while( iter.hasNext() ) {
			...
			if ( "key".equals(name) || "generated-key".equals(name) ) {
				...
				bindValue(subnode, key, model.isOneToMany(), Collection.DEFAULT_KEY_COLUMN_NAME);
				...
				bindValue(subnode, elt, true, Collection.DEFAULT_ELEMENT_COLUMN_NAME);
			}
			else if ( "many-to-many".equals(name) ) {
				ManyToOne element = new ManyToOne( model.getTable() );
				...
				CacheConcurrencyStrategy cache = createCache( subnode, model.getRole(), model.getOwner() );
		...
		final void doSecondPass(java.util.Map persistentClasses) throws MappingException {
			...
			if ( log.isDebugEnabled() ) {
				...
				if ( collection.isOneToMany() ) {
					msg += ", one-to-many: " + collection.getOneToMany().getType().getName();
				}
				else {
	...
	//TODO: move to a new CacheFactory class!
	public static CacheConcurrencyStrategy createCache(Element node, String name, PersistentClass owner) throws MappingException {
		...
		if ( usage.equals("read-only") ) {
			...
			throw new MappingException("cache usage attribute should be read-write or read-only");
		}
		return ccs;
	}
