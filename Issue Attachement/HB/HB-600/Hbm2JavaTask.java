...
public class Hbm2JavaTask extends Task {
	...
	public void execute() throws BuildException {
		...
		try {
			log("Building hibernate objects");
                        processFiles(outputDir, fileList);
		} catch (Throwable t) {
	...
	private void processFiles (File outputDir, List fileList) {
		...
		if (configurationFile != null) {
			args.add("--config=" + configurationFile);
                ...
                for (int i = 0; i < fileList.size(); i++) {
                    File targetFile = (File) fileList.get(i);
                    args.add(targetFile.getAbsoluteFile().toString());
                }
