 ...
 * @version 1.0
 */
public class Finder2Renderer extends AbstractRenderer {

   private static final String MT_FINDERMETHOD = "finder-method";
   private static final String MT_FINDERUNIQUE = "finder-unique";
   ...
   private static final String MT_FOREIGNJOINFIELD = "foreign-join-field";
    ...
    private static Log log = LogFactory.getLog(Finder2Renderer.class);
    ...
    public void doFinders(ClassMapping classMapping, Map class2classmap, PrintWriter writer) {
        ...
        for (Iterator fields = classMapping.getFields().iterator(); fields.hasNext(); ) {
            ...
            if (field.getMeta(MT_FINDERMETHOD) != null) {
                ...
                writer.println("    }");
            writer.println();
			} else if (field.getMeta(MT_FINDERUNIQUE) != null) {

				 // return a unique instance of the object.  Return null
				 // if no objects found.  Return the object if one object
				 // was found.  Throw an exception if more than one object
				 // was found.

				 String finderName = field.getMetaAsString(MT_FINDERUNIQUE);
				 String className = classMapping.getName();

				 writer.print("    public static " + className + " " +
					 finderName + "(");
				 ...
				 writer.println(JavaTool.getTrueTypeName(field, class2classmap) +
					 " " + field.getFieldName() +
					 ") throws SQLException, HibernateException {");
				 ...
				 writer.println("        List finds = session.find(\"from " +
					classMapping.getFullyQualifiedName()
					+ " as " + classMapping.getName().toLowerCase()
					+ " where " + classMapping.getName().toLowerCase() + "." + field.getFieldName() + "=?\", " + getFieldAsObject(false, field)
							+ ", " + getFieldAsHibernateType(false, field) + ");");

				 writer.println("        if (finds.size() == 0) {");
				 writer.println("            return null;");
				 writer.println("        } else if (finds.size() == 1) {");
				 writer.println("            return (" + className +
					 ") finds.get(0);");
				 writer.println("        } else {");
				 writer.println("            throw new NonUniqueResultException(" +
				    "finds.size());");
				 writer.println("        }");
				 writer.println("    }");
            writer.println();
