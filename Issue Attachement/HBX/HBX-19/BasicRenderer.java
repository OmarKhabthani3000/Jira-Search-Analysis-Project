//$Id: BasicRenderer.java,v 1.16.2.3 2004/04/25 11:22:21 maxcsaucdk Exp $
...
public class BasicRenderer extends AbstractRenderer {
   ...
   /* public void doFieldNameConstants(ClassMapping classMapping, PrintWriter writer) {
	...
	public void doToString(ClassMapping classMapping, PrintWriter writer) {
		...
		classMapping.addImport("org.apache.commons.lang.builder.StandardToStringStyle");	
        writer.println("    private static final HibernateToStringStyle HIBERNATE_TO_STRING_STYLE = new HibernateToStringStyle();");
        writer.println();
        writer.println("    private final class HibernateToStringStyle extends StandardToStringStyle {");
        writer.println("        public HibernateToStringStyle() {");
        writer.println("            super();");
        writer.println("            setUseIdentityHashCode(false);");
        writer.println("        }");
        writer.println("    }");
        writer.println();
		...
		writer.println("        return new ToStringBuilder(this, HIBERNATE_TO_STRING_STYLE)");
		for (Iterator fields = classMapping.getAllFields().iterator(); fields.hasNext();) {
			FieldProperty field = (FieldProperty) fields.next();
