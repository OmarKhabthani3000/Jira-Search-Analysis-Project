//$Id: AbstractCollectionPersister.java,v 1.24 2005/05/21 16:22:37 oneovthafew Exp $
...
public abstract class AbstractCollectionPersister
	...
	protected final boolean[] elementColumnIsNotNullable;
	...
	throws MappingException, CacheException {
		...
		while ( iter.hasNext() ) {
			...
			keyColumnAliases[k] = col.getAlias();
			k++;
		}
		...
		elementColumnIsNotNullable = new boolean[elementSpan];
		...
		while ( iter.hasNext() ) {
			...
			elementColumnAliases[j] = s.getAlias();
			if ( s.isFormula() ) {
				...
				elementColumnIsNotNullable[j] = !col.isNullable();
		...
		//row locator SQL
		if ( !hasNotNullableColumns ) Arrays.fill(elementColumnIsNotNullable, true);
		...
		if (hasIndex) {
			...
			while ( iter.hasNext() ) {
				Selectable s = (Selectable) iter.next();
				indexColumnAliases[i] = s.getAlias();
				if ( s.isFormula() ) {
		...
		if (hasIdentifier) {
			...
			identifierColumnAlias = col.getAlias();
	...
	protected int writeElementToWhere(PreparedStatement st, Object elt, int i, SessionImplementor session)
		...
		getElementType().nullSafeSet(st, elt, i, elementColumnIsNotNullable, session);
