...
public class JPAUnitTestCase {
	...
	public void hhh123Test() throws Exception {
		final org.hibernate.mapping.Table sublclassTable = getTable(LANGUAGE_TABLE_NAME);
		final org.hibernate.mapping.Table associatedTable = getTable(B2L_TABLE_NAME);

		Assert.assertEquals("Expected to have no foreign keys in associated table '" + B2L_TABLE_NAME + "'",
				Collections.emptyMap(), associatedTable.getForeignKeys());

		Assert.assertEquals("Expected to have no foreign keys in subclass table '" + LANGUAGE_TABLE_NAME + "'",
				Collections.emptyMap(),
				sublclassTable.getForeignKeys());
	}
	...
	private org.hibernate.mapping.Table getTable(final String tableName) {
		...
		Assert.assertNotNull("Expected to find subclass table '" + tableName + "' in Mapping", res);
		return res;
	}

	private org.hibernate.mapping.Table getTable0(final String tableName) {
		for (final Namespace namespace : MetadataExtractorIntegrator.INSTANCE.database.getNamespaces()) {
