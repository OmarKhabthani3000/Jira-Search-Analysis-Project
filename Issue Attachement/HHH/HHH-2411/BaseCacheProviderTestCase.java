...
public abstract class BaseCacheProviderTestCase extends TestCase {
	...
	public void testStaleWritesLeaveCacheConsistent() {
		...
		//Exception org.jboss.cache.optimistic.DataVersioningException during the commit
		txn.commit();
		...
		s = openSession();
		txn = s.beginTransaction();
		item = (VersionedItem) s.get( VersionedItem.class, item.getId() );

		entry = slcs.getEntries().get( item.getId() );
		Map map;
		if ( entry instanceof ReadWriteCache.Item ) {
			map = new HashMap();
			map = (Map) ( (ReadWriteCache.Item) entry ).getValue();
		...
		assertTrue( map.get("description").equals("steve's item") );
		assertTrue( map.get("name").equals("steve") );
	...
	public void testQueryCacheInvalidationVersionedItem() {
		Session s = openSession();
		Transaction t = s.beginTransaction();
		VersionedItem item = new VersionedItem();
		item.setName("widget");
