...
/**
 * Delegate that handles the type and join sequence information for a
 * FromElement.
 *
...
class FromElementType {
	...
	public FromElementType(FromElement fromElement, EntityPersister persister,
			EntityType entityType) {
		this.fromElement = fromElement;
		this.persister = persister;
		this.entityType = entityType;
		if (persister != null) {
			fromElement.setText(((Queryable) persister).getTableName() + " "
					+ getTableAlias());
		}
	}
	...
	public Type getSelectType() {
		if (entityType == null)
			return null;
		boolean shallow = fromElement.getFromClause().getWalker()
				.isShallowQuery();
		return TypeFactory.manyToOne(entityType.getAssociatedEntityName(),
				shallow);
	}
	...
	/**
	 * Render the identifier select, but in a 'scalar' context (i.e. generate
	 * the column alias).
	 *
	 * @param i
	...
	String renderScalarIdentifierSelect(int i) {
		checkInitialized();
		String[] cols = getPropertyMapping(EntityPersister.ENTITY_ID)
				.toColumns(getTableAlias(), EntityPersister.ENTITY_ID);
		...
		for (int j = 0; j < cols.length; j++) {
			...
			buf.append(column).append(" as ").append(
					NameGenerator.scalarName(i, j));
		}
		return buf.toString();
	}
	...
	String renderIdentifierSelect(int size, int k) {
		...
		if (fromElement.getFromClause().isSubQuery()) {
			...
			String[] idColumnNames = (persister != null) ? ((Queryable) persister)
					.getIdentifierColumnNames()
					: new String[0];
			StringBuffer buf = new StringBuffer();
			for (int i = 0; i < idColumnNames.length; i++) {
				buf.append(fromElement.getTableAlias()).append('.').append(
						idColumnNames[i]);
			...
			String fragment = ((Queryable) persister).identifierSelectFragment(
					getTableAlias(), getSuffix(size, k));
	...
	String renderPropertySelect(int size, int k, boolean allProperties) {
		...
		if (persister == null) {
			...
			String fragment = ((Queryable) persister).propertySelectFragment(
					getTableAlias(), getSuffix(size, k), allProperties);
			return trimLeadingCommaAndSpaces(fragment);
		}
	}
	...
	String renderCollectionSelectFragment(int size, int k) {
		if (queryableCollection == null) {
			...
			String fragment = queryableCollection.selectFragment(
					getCollectionTableAlias(), collectionSuffix);
			return trimLeadingCommaAndSpaces(fragment);
		}
	}
	...
	public String renderValueCollectionSelectFragment(int size, int k) {
		if (queryableCollection == null) {
			...
			String fragment = queryableCollection.selectFragment(
					getTableAlias(), collectionSuffix);
			return trimLeadingCommaAndSpaces(fragment);
		}
	}

	/**
	 * This accounts for a quirk in Queryable, where it sometimes generates ', '
	 * in front of the SQL fragment. :-P
	 *
	 * @param fragment
	 *            An SQL fragment.
	...
	public JoinSequence getJoinSequence() {
		...
		// Class names in the FROM clause result in a JoinSequence (the old
		// FromParser does this).
		if (persister instanceof Joinable) {
			Joinable joinable = (Joinable) persister;
			return fromElement.getSessionFactoryHelper().createJoinSequence()
					.setRoot(joinable, getTableAlias());
		} else {
			return null; // TODO: Should this really return null? If not,
	...
	public void setQueryableCollection(QueryableCollection queryableCollection) {
		if (this.queryableCollection != null) {
			throw new IllegalStateException(
					"QueryableCollection is already defined for " + this + "!");
		}
		this.queryableCollection = queryableCollection;
		if (!queryableCollection.isOneToMany()) {
			// For many-to-many joins, use the tablename from the queryable
			// collection for the default text.
			fromElement.setText(queryableCollection.getTableName() + " "
					+ getTableAlias());
		}
	}
	...
	/**
	 * Returns the type of a property, given it's name (the last part) and the
	 * full path.
	 *
	 * @param propertyName
	...
	public Type getPropertyType(String propertyName, String propertyPath) {
		checkInitialized();
		Type type = null;
		// If this is an entity and the property is the identifier property,
		// then use getIdentifierType().
		// Note that the propertyName.equals( propertyPath ) checks whether we
		// have a component
		// key reference, where the component class property name is the same as
		// the
		// entity id property name; if the two are not equal, this is the case
		// and
		// we'd need to "fall through" to using the property mapping.
		if (persister != null && propertyName.equals(propertyPath)
				&& propertyName.equals(persister.getIdentifierPropertyName())) {
		...
		if (type == null) {
			throw new MappingException("Property "
					...
					+ " "
					+ ((queryableCollection == null) ? fromElement
							.getClassName() : queryableCollection.getRole()));
		}
		return type;
	}
			...
			if (forceAlias) {
				...
				if (isCorrelation()) {
					if (isMultiTable()) {
						...
						return propertyMapping.toColumns(extractTableName(),
								path);
					}
				} else {
					return propertyMapping.toColumns(tableAlias, path);
				}
			//START - FIXING BUG ""ORA-00918: column ambiguously defined" on MERGE/UPDATE when using Joined Subclasses strategy." - START
			} else if (fromElement.getWalker().getStatementType() == HqlSqlTokenTypes.UPDATE //BUG FIX - This line has been included
					&& fromElement.getEntityPersister() instanceof JoinedSubclassEntityPersister //BUG FIX - This line has been included
					&& fromElement.getWalker().getCurrentClauseType() == HqlSqlTokenTypes.WHERE) { //BUG FIX - This line has been included
				return propertyMapping.toColumns(tableAlias, path); //BUG FIX - This line has been included
			} else if (fromElement.getWalker().getStatementType() == HqlSqlTokenTypes.DELETE //BUG FIX - This line has been included
					&& fromElement.getEntityPersister() instanceof JoinedSubclassEntityPersister //BUG FIX - This line has been included
					&& fromElement.getWalker().getCurrentClauseType() == HqlSqlTokenTypes.WHERE) { //BUG FIX - This line has been included
				return propertyMapping.toColumns(tableAlias, path); //BUG FIX - This line has been included
			//END - FIXING BUG ""ORA-00918: column ambiguously defined" on MERGE/UPDATE when using Joined Subclasses strategy." - END
			} else {
	...
	private boolean isCorrelation() {
		...
		return fromElement.getFromClause() != fromElement.getWalker()
	...
	PropertyMapping getPropertyMapping(String propertyName) {
		...
		if (queryableCollection == null) { // Not a collection?
			return (PropertyMapping) persister; // Return the entity property
												// mapping.
		}
		// If the property is a special collection property name, return a
		// CollectionPropertyMapping.
		if (CollectionProperties.isCollectionProperty(propertyName)) {
			if (collectionPropertyMapping == null) {
				collectionPropertyMapping = new CollectionPropertyMapping(
						queryableCollection);
			}
		...
		if (queryableCollection.getElementType().isComponentType()) {
			...
			if (propertyName.equals(EntityPersister.ENTITY_ID)) {
				return (PropertyMapping) queryableCollection
