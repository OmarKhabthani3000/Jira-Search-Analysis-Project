...
public class CompoundNaturalIdCacheTest extends BaseCoreFunctionalTestCase {
  ...
  @Test
  public void createThenLoadTest() throws Exception {
    ...
    log.infov("Persisted " + objectsNb + ' ' + EntityWithCompoundNaturalId.class.getSimpleName() +
    ...
    log.infov("Persisted " + objectsNb + ' ' + EntityWithSimpleNaturalId.class.getSimpleName() +
            " objects, duration=" + creationDurationForSimpleNaturalId + "ms");

    tx.commit();
    s.close();

    // creations of EntityWithCompoundNaturalId are really slow as the naturalId cache is updated too
  ...
  private long loadEntities(final Class<?> clazz, final int maxResults) {
    ...
    var start = System.currentTimeMillis();
    log.infov("Loading at most " + maxResults + "  instances of " + clazz);
    ...
    log.infov("Loaded " + objects.size() + " instances of " + clazz + ", duration=" + duration + "ms");
