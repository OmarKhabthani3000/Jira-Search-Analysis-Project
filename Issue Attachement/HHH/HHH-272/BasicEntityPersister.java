//$Id: BasicEntityPersister.java,v 1.10 2005/02/24 20:27:54 steveebersole Exp $
...
public abstract class BasicEntityPersister
    ...
    // SJH
    private final String[][] propertyColumnGetters;
    ...
    // SJH
    private final String[] subclassColumnGetterClosure;
    private final String[] subclassColumnSetterClosure;
	...
	throws HibernateException {
		...
		// SJH
		propertyColumnGetters = new String[hydrateSpan][];
		...
		while ( iter.hasNext() ) {
			...
			// SJH
			String[] colGetters = new String[span];
			...
			while ( colIter.hasNext() ) {
				...
				// SJH
				if (thing instanceof Column) {
				    colGetters[k] = ((Column) thing).getGetter();
				    colSetters[k] = ((Column) thing).getSetter();
				}
			...
			// SJH
			propertyColumnGetters[i] = colGetters;
		...
		// SJH
		ArrayList columnGetters = new ArrayList();
		ArrayList columnSetters = new ArrayList();
		...
		while ( iter.hasNext() ) {
			...
			while ( colIter.hasNext() ) {
				...
				// SJH
				if (thing instanceof Column) {
				    columnGetters.add (((Column) thing).getGetter ());
				    columnSetters.add (((Column) thing).getSetter ());
				}
		...
		// SJH
		subclassColumnGetterClosure = ArrayHelper.toStringArray (columnGetters);
		subclassColumnSetterClosure = ArrayHelper.toStringArray (columnSetters);
    ...
    // SJH
    protected String[] getSubclassColumnGetters() {
	return subclassColumnGetterClosure;
    }
    // SJH
    protected String[] getSubclassColumnSetters() {
	return subclassColumnSetterClosure;
    }
	...
	public String propertySelectFragment(String name, String suffix) {
		...
		// SJH
		String[] getters = getSubclassColumnGetters();
		...
		for ( int i = 0; i < getSubclassColumnClosure().length; i++ ) {
			...
			if ( selectable ) {
				...
				if (getter != null)
				    select.setGetter (subalias, columns[i], getter);
	...
	protected String concretePropertySelectFragment(String alias, boolean[] includeProperty) {
		...
		for ( int i = 0; i < propertyCount; i++ ) {
			if ( includeProperty[i] ) { //ie. updateable, not a formula
				...
				// SJH
				frag.setSqlGetters (generateTableAlias(alias, propertyTableNumbers[i]),
						    propertyColumnNames[i],
						    propertyColumnGetters[i]);
    ...
    // SJH
    private String[] getPropertyColumnGetters(int i) {
	return propertyColumnGetters[i];
    }
    // SJH
    private String[] getPropertyColumnSetters(int i) {
	...
	public int getPropertyIndex(String propertyName) {
		return ( (Integer) propertyIndexes.get(propertyName) ).intValue();
	}
	...
	private void initIdentifierPropertyPaths(Mapping mapping) throws MappingException {
		...
		if ( idProp != null ) {
			propertyMapping.initPropertyPaths( idProp, getIdentifierType(), getIdentifierColumnNames(), mapping );
		}
		if ( entityMetamodel.getIdentifierProperty().isEmbedded() ) {
			propertyMapping.initPropertyPaths( null, getIdentifierType(), getIdentifierColumnNames(), mapping );
		}
		propertyMapping.initPropertyPaths( ENTITY_ID, getIdentifierType(), getIdentifierColumnNames(), mapping );
	}
	...
	protected String generateUpdateString(final boolean[] includeProperty,
		...
		for ( int i = 0; i < entityMetamodel.getPropertySpan(); i++ ) {
			if ( includeProperty[i] && isPropertyOfTable( i, j ) ) {
				...
				// SJH
				update.setSqlSetters (getPropertyColumnNames(i), getPropertyColumnSetters(i));
	...
	protected String generateInsertString(boolean identityInsert, boolean[] includeProperty, int j) {
		...
		for ( int i = 0; i < entityMetamodel.getPropertySpan(); i++ ) {
			if ( includeProperty[i] && isPropertyOfTable( i, j ) ) {
				...
				// SJH
				insert.setSqlSetters (getPropertyColumnNames(i), getPropertyColumnSetters(i));
			}
		}
