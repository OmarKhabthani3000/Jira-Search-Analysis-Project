// $Id: HbmBinder.java,v 1.67 2005/02/21 04:29:24 oneovthafew Exp $
...
public final class HbmBinder {
	...
	public static void bindUnionSubclass(Element node, UnionSubclass unionSubclass,
		...
		Table mytable = mappings.addDenormalizedTable( schema, catalog, getClassTableName(
			unionSubclass,
			node,
			mappings ), unionSubclass.isAbstract(), getSubselect( node ), unionSubclass
			.getSuperclass()
			.getTable() );
		...
		if ( columnAttribute == null ) {
			...
			while ( iter.hasNext() ) {
				...
				if ( columnElement.getName().equals( "column" ) ) {
					...
					// SJH
					bindSqlGet (columnElement.attribute("sql-get"), table, column);
					bindSqlSet (columnElement.attribute("sql-set"), table, column);
				}
			...
			bindIndex( node.attribute( "index" ), table, column );
			// SJH
			bindSqlGet(node.attribute("sql-get"), table, column);
			bindSqlSet(node.attribute("sql-set"), table, column);
		}
    ...
    // SJH
    private static void bindSqlGet(Attribute getAttribute, Table table, Column column) {
	if (getAttribute != null && column != null) {
	    column.setGetter(getAttribute.getValue());
	}
    }

    // SJH
    private static void bindSqlSet(Attribute setAttribute, Table table, Column column) {
	if (setAttribute != null && column != null) {
	    column.setSetter(setAttribute.getValue());
	}
    }
	...
	public static void bindColumn(Element node, Column column, boolean isNullable) {
		...
		if ( scalNode != null ) column.setLength( Integer.parseInt( scalNode.getValue() ) );
	...
	private static void makeIdentifier(Element node, SimpleValue model, Mappings mappings) {
		...
		if ( subnode != null ) {
			...
			if ( mappings.getCatalogName() != null ) {
				params.setProperty( PersistentIdentifierGenerator.CATALOG, mappings
					.getCatalogName() );
			}
			params.setProperty( PersistentIdentifierGenerator.TABLE, model.getTable().getName() );

			params.setProperty( PersistentIdentifierGenerator.PK, ( (Column) model
				.getColumnIterator()
				.next() ).getName() );
	...
	protected static void createClassProperties(Element node, PersistentClass persistentClass,
		...
		while ( iter.hasNext() ) {
			...
			if ( collectType != null ) {
				...
				parseFilter( subnode, persistentClass );
			}
	...
	private static void bindReturn(Element returnElem, java.util.List queryReturns,
		...
		queryReturns.add( new SQLQueryRootReturn(
			alias,
			getClassName( classAttribute, mappings ),
			lockMode ) );
	}
	...
	private static void parseFilterDef(Element element, Mappings model) {
		String name = element.attributeValue( "name" );
		log.debug( "Parsing filter-def [" + name + "]" );
		FilterDefinition config = new FilterDefinition( name );
		...
		while ( params.hasNext() ) {
			...
			config.addParameterType( paramName, heuristicType );
		}
		log.debug( "Parsed filter-def [" + name + "]" );
		model.addFilterDefinition( config );
	}

	private static void parseFilter(Element filterElement, Filterable filterable) {
	...
	private static void findClassNames(Mappings mappings, final Element startNode,
		...
		while ( classIterator.hasNext() ) {
			...
			String name = getClassName( element.attributeValue( "name" ), mappings );
			// TODO: support entity-name ?
			names.add( name );
