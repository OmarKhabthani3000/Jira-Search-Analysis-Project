//$Id: AbstractCollectionPersister.java,v 1.10 2005/02/22 13:25:08 oneovthafew Exp $
...
public abstract class AbstractCollectionPersister
    ...
    //SJH
    private final String[] elementColumnGetters;
    private final String[] elementColumnSetters;
	...
	throws MappingException, CacheException {
		...
		while ( iter.hasNext() ) {
			...
			if (s instanceof Column) {
			    elementColumnGetters[j] = ((Column) s).getGetter();
			    elementColumnSetters[j] = ((Column) s).getSetter();
			}
	...
	protected CollectionInitializer getAppropriateInitializer(Serializable key, SessionImplementor session) {
		if ( queryLoaderName != null ) return initializer;
		CollectionInitializer subqueryInitializer = getSubqueryInitilizer( key, session );
		if ( subqueryInitializer != null ) return subqueryInitializer;
		if ( session.getEnabledFilters().isEmpty() ) return initializer;
		return createCollectionInitializer( session.getEnabledFilters() );
	}

	private CollectionInitializer getSubqueryInitilizer(Serializable key, SessionImplementor session) {
		
		if ( !isOneToMany() ) return null; //TODO: WHY???!
		
		final PersistenceContext persistenceContext = session.getPersistenceContext();
		
		Subquery subquery = persistenceContext.getBatchFetchQueue()
			.getSubquery( new EntityKey( key, getOwnerEntityPersister(), session.getEntityMode() ) );
		
		if (subquery == null) return null;
		
		// Take care of any entities that might have
		// been evicted!	
		Iterator iter = subquery.getResult().iterator();
		while ( iter.hasNext() ) {
		...
		return getSubqueryInitializer( subquery, session );
	}

	protected abstract CollectionInitializer getSubqueryInitializer(Subquery subquery, SessionImplementor session);
	...
	public String selectFragment(String alias) {
		...
		for ( int i=0; i<elementColumnIsSettable.length; i++ ) {
			...
			//SJH
			String getter = elementColumnGetters[i];
			if (getter != null)
			    frag.setGetter (alias, elementColumnNames[i], getter);
		}
	...
	public boolean isAffectedByEnabledFilters(SessionImplementor session) {
		for ( int i = 0, max = filterNames.length; i < max; i++ ) {
			if ( session.getEnabledFilters().containsKey( filterNames[i] ) ) {
				return true;
			}
		}
		return false;
