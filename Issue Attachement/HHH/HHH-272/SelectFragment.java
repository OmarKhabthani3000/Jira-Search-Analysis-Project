...
public class SelectFragment {
    ...
    // SJH
    public SelectFragment setGetter(String tableAlias, String columnName, String getter) {
	String c = StringHelper.qualify (tableAlias, columnName);
	getters.put (c, StringHelper.replace (getter, Template.TEMPLATE, c));
	return this;
    }
    // SJH
    public SelectFragment setSqlGetters (String tableAlias, String[] columnNames, String[] getters) {
	for (int i=0; i<columnNames.length; i++) {
	    String getter = getters [i];
	    if (getter != null) this.setGetter (tableAlias, columnNames[i], getter);
	}
	return this;
    }
		...
		while ( iter.hasNext() ) {
			...
			/*if ( otherAlias!=null && !columnAlias.equals(otherAlias) ) {
				throw new AssertionFailure("bug in Hibernate SQL alias generation");
                        ...
                        // SJH
			String getter = (String) getters.get (column);

			if ( columnsUnique.add(columnAlias) ) {
                                // SJH
				buf.append(", ")
					.append((getter == null) ? column : getter)
					.append(" as ");
				if (suffix==null) {
