 ...
 *
 * License: GNU Lesser General Public License (LGPL), version 2.1 or later. See the lgpl.txt file in the root directory or
...
public class Ejb3Column {
    ...
    // private static final CoreMessageLogger LOG = Logger.getLogger(CoreMessageLogger.class.getName(),
    // Ejb3Column.class.getName());
                ...
                if (propertyName != null) {
                    ...
                    if (implicitName.getText().contains("_collection&&element_")) {
                        implicitName = Identifier
                            .toIdentifier(implicitName.getText().replace("_collection&&element_", "_"), implicitName.isQuoted());
                    }
    ...
    protected void addColumnBinding(SimpleValue value) {
        ...
        if (StringHelper.isNotEmpty(this.logicalColumnName)) {
            ...
            final Identifier implicitName = normalizer
                .normalizeIdentifierQuoting(implicitNamingStrategy.determineBasicColumnName(new ImplicitBasicColumnNameSource() {
    ...
    public static Ejb3Column[] buildColumnFromAnnotation(
        ...
        Ejb3Column[] columns;
        if (formulaAnn != null) {
            final Ejb3Column formulaColumn = new Ejb3Column();
            ...
            final String path = StringHelper.qualify(propertyHolder.getPath(), inferredData.getPropertyName());

            javax.persistence.Column[] actualCols = anns;
            // final javax.persistence.Column[] overriddenCols = propertyHolder.getOverriddenColumn(path);
            final javax.persistence.Column[] overriddenCols = getOverriddenColumns(propertyHolder, path, actualCols);
            ...
            if (actualCols == null) {
                ...
                for (int index = 0; index < length; index++) {
                    ...
                    final ObjectNameNormalizer normalizer = context.getObjectNameNormalizer();
                    final Database database = context.getMetadataCollector().getDatabase();
                    final ImplicitNamingStrategy implicitNamingStrategy = context
                        .getBuildingOptions()
                        .getImplicitNamingStrategy();
                    final PhysicalNamingStrategy physicalNamingStrategy = context
                        .getBuildingOptions()
                        .getPhysicalNamingStrategy();

                    final javax.persistence.Column col = actualCols[index];
                    ...
                    final Ejb3Column column = new Ejb3Column();
    ...
    private static void applyColumnDefault(Ejb3Column column, PropertyData inferredData) {
        ...
        if (xProperty != null) {
            final ColumnDefault columnDefaultAnn = xProperty.getAnnotation(ColumnDefault.class);
    ...
    private void extractDataFromPropertyData(PropertyData inferredData) {
        if (inferredData != null) {
            ...
            if (property != null) {
                ...
                if (annotations != null) {
                    for (final ColumnTransformer annotation : annotations.value()) {
    ...
    private void processExpression(ColumnTransformer annotation) {
        final String nonNullLogicalColumnName = logicalColumnName != null ? logicalColumnName : ""; // use the default for
                                                                                                    // annotations
        if (annotation != null
            && (StringHelper.isEmpty(annotation.forColumn()) || annotation.forColumn().equals(nonNullLogicalColumnName))) {
    ...
    private static Ejb3Column[] buildImplicitColumn(
        ...
        MetadataBuildingContext context) {
        final Ejb3Column column = new Ejb3Column();
        final Ejb3Column[] columns = new Ejb3Column[1];
        columns[0] = column;
    ...
    public static void checkPropertyConsistency(Ejb3Column[] columns, String propertyName) {
        final int nbrOfColumns = columns.length;
    ...
    void addIndex(String indexName, boolean inSecondPass) {
        final IndexOrUniqueKeySecondPass secondPass = new IndexOrUniqueKeySecondPass(indexName, this, context, false);
        if (inSecondPass) {
    ...
    void addUniqueKey(String uniqueKeyName, boolean inSecondPass) {
        final IndexOrUniqueKeySecondPass secondPass = new IndexOrUniqueKeySecondPass(uniqueKeyName, this, context, true);
        if (inSecondPass) {
            secondPass.doSecondPass(context.getMetadataCollector().getEntityBindingMap());
        } else {
            context.getMetadataCollector().addSecondPass(secondPass);
