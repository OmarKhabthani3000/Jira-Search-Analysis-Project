   ...
   throws SQLException, HibernateException {
      ...
      if ( translator.hasProjection() ) {
         ...
         for ( int i=0; i<result.length; i++ ) {
            ...
            /*
             * cbono: Added following fix to handle component projections.
             * -----------------------------------------------------------
             * If the type being processed is a ComponentType then pass 
             * in the columnAliases slice of array for the numColumns in 
             * the component. Previously the core loader was just passing
             * in the very first alias, which fails for components that
             * have multiple aliases/properties.
             *
             * Example:
             * 
             * ProjectionList
             *     type                   aliases to be passed in
             *     -------------          -----------------------
             *     StringType             y0_
             *     ComponentType(2 props) y1_, y2_
             *     StringType             y3_
             */            
            String[] actualColAliases = null;
            
            if (types[i] instanceof ComponentType)
            {
               ComponentType comp = (ComponentType) types[i];
               int numProps = comp.getPropertyNames().length;
               actualColAliases = new String[numProps];
               
               for (int j = 0; j < numProps; j++)
               {
                  actualColAliases[j] = columnAliases[curAliasIdx];
                  curAliasIdx++;
               }
            }
            else
            {
               actualColAliases = new String[] {columnAliases[i]};
               curAliasIdx++;
            }
            result[i] = types[i].nullSafeGet(rs, actualColAliases, session, null);            
         }
