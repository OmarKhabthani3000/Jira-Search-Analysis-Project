...
public class JTASessionContext implements CurrentSessionContext {
	...
	public Session currentSession() throws HibernateException {
		...
		Session currentSession = ( Session ) currentSessionMap.get( String.valueOf(txn.hashCode()) );
      ...
      String txnCode = String.valueOf(txn.hashCode());

      if (log.isDebugEnabled())
        log.debug("####  Creating session for transaction '"+ txnCode +"'; current session map (size="+ currentSessionMap.size() +")");

      currentSessionMap.put( txnCode, currentSession );
		}
	...
	private CleanupSynch buildCleanupSynch(Transaction txn) {
		return new CleanupSynch( String.valueOf(txn.hashCode()), this );
	}
...
//		}
    ...
    public CleanupSynch(String txnCode, JTASessionContext context) {
    ...
    public void beforeCompletion() {
      ...
      if (log.isDebugEnabled())
        log.debug("#### Removing transaction '"+ txnCode +"' from current session map (size="+ context.currentSessionMap.size() +")");

      context.currentSessionMap.remove( txnCode );

      if (log.isDebugEnabled())
        log.debug("#### Transaction removed. Current session map count is '"+ context.currentSessionMap.size() +"'");
