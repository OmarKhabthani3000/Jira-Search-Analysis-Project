...
public class DB2Dialect extends Dialect {
	...
	public DB2Dialect() {
		...
		registerFunction( "current_timestamp", new NoArgSQLFunction( "current timestamp", StandardBasicTypes.TIMESTAMP,
				false ) );
	...
	public String getLimitString(String sql, int offset, int limit) {
		...
		StringBuilder pagingSelect = new StringBuilder( sql.length() + 200 )
				...
				.append( " fetch first " ).append( limit )
	 ...
	 *            The user-supplied, zero-based offset
	 * 
	...
	public static void main(String[] args) {
		...
		System.out.println( new DB2Dialect().getLimitString(
				"/*foo*/ select distinct * from foos foo order by foo.bar, foo.baz", true ) );
	}
	 ...
	 * {@inheritDoc}
	 * <p/>
	 * DB2 is know to support parameters in the <tt>SELECT</tt> clause, but only in casted form (see
	 * {@link #requiresCastingOfParametersInSelectClause()}).
	 * 
	 * @return True.
	 */
	@Override
	...
	protected SqlTypeDescriptor getSqlTypeDescriptorOverride(int sqlCode) {
		return sqlCode == Types.BOOLEAN ? SmallIntTypeDescriptor.INSTANCE : super
				.getSqlTypeDescriptorOverride( sqlCode );
	}
