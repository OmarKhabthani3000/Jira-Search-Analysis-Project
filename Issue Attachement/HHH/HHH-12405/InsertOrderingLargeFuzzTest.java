	...
	private void fuzzTest(
		...
		doInHibernate(this::sessionFactory,
			session -> {
				...
				// Persist all the entities in a semi-random order that is
				...
				int	   sortRandomness = setSize / 4; // TODO Experiment and
														// find the value for
														// this [2, 4, 8] that
														// maximizes the
														// amount of work
				...
				for (int i = 0; i < setSize; i++) {
					...
					session.persist(classObject[(setSize - 1) - x]);		   // TODO Test which way around produces better results
					log.warn(
						"Persisted classObject[" + ((setSize - 1) - x) + "]"); // TODO Remove
				}
