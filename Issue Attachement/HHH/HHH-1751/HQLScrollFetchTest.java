...
public class HQLScrollFetchTest extends TestCase {
	...
	private static final String QURY = "select p from Parent p join fetch p.children c";
	...
	public void testNoScroll () {
		try {
			...
			List list = s.createQuery(QURY).setResultTransformer(new DistinctRootEntityResultTransformer()).list();
	...
	public void testScroll () {
		try {
			insertTestData();
			Session s = openSession();
			ScrollableResults results = s.createQuery(QURY).scroll();
	...
	public void testScrollOrderParentAsc () {
		try {
			insertTestData();
			Session s = openSession();
			ScrollableResults results = s.createQuery(QURY+" order by p.name asc").scroll();
			List list = new ArrayList();
			while (results.next()) {
				list.add(results.get(0));
			}
	...
	public void testScrollOrderParentDesc () {
		try {
			insertTestData();
			Session s = openSession();
			ScrollableResults results = s.createQuery(QURY+" order by p.name desc").scroll();
			List list = new ArrayList();
			while (results.next()) {
				list.add(results.get(0));
			}
	...
	public void testScrollOrderParentAscChildrenAsc () {
		try {
			insertTestData();
			Session s = openSession();
			ScrollableResults results = s.createQuery(QURY+" order by p.name asc, c.name asc").scroll();
			List list = new ArrayList();
			while (results.next()) {
				list.add(results.get(0));
			}
	...
	public void testScrollOrderParentAscChildrenDesc () {
		try {
			insertTestData();
			Session s = openSession();
			ScrollableResults results = s.createQuery(QURY+" order by p.name asc, c.name desc").scroll();
			List list = new ArrayList();
			while (results.next()) {
				list.add(results.get(0));
			}
