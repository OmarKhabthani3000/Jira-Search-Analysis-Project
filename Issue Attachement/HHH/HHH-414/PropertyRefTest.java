...
public class PropertyRefTest extends TestCase {
    ...
    /**
	 * Test for a regression in EntityType.loadByUniqueKey()
	 * See http://opensource.atlassian.com/projects/hibernate/browse/HHH-414 for details
	 *
	 */
	public void testHHH414regression() {
		Session s = openSession();
		Transaction t = s.beginTransaction();
		Person p = new Person();
		p.setName("Steve");
		...
		boolean statisticsAlreadyEnabled = isStatisticsEnabled();
		if (!statisticsAlreadyEnabled) {
	...
	private boolean isStatisticsEnabled() {
		return getSessions().getStatistics().isStatisticsEnabled();
	}

	private void setStatisticsEnabled(boolean enabled) {
		getSessions().getStatistics().setStatisticsEnabled(enabled);
	}
	...
	private void assertStatementCount(long count) {
		long statements = getSessions().getStatistics().getPrepareStatementCount();
		assertEquals("Number of database hits doesn't match expected!", count, statements);
	}
