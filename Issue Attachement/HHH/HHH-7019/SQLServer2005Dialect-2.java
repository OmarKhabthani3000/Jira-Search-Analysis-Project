...
public class SQLServer2005Dialect extends SQLServerDialect {
	...
	public String getLimitString(String querySqlString, boolean hasOffset) {
		...
		int orderByIndex = shallowIndexOf(sb, "order by");
	...
	protected static void replaceDistinctWithGroupBy(StringBuilder sql) {
		int distinctIndex = shallowIndexOf(sql, DISTINCT);
		if (distinctIndex > 0) {
	...
	protected static CharSequence getSelectFieldsWithoutAliases(StringBuilder sql) {
		String select = sql.substring(shallowIndexOf(sql, SELECT) + SELECT.length(), shallowIndexOf(sql, FROM));
	...
	/**
	 * Searches a string for another string, returns the offset of the first match,
	 * and prevents matching when the search term is found inside of parenthesis.
	 *
	 * @param sb			String to search
	 * @param search	Search term
	 * @return        Position of the first match, or -1 if no match
	 */
	protected static int shallowIndexOf(StringBuilder sb, String search) {
		char c;
		int pos, depth = 0, iter, cur = 0, len, searchlen;

		String lowercase = sb.toString().toLowerCase(); // case-insensitive match
		len = lowercase.length(); searchlen = search.length();
		do {
			pos = lowercase.indexOf(search, cur);
			if (pos != -1) {
				for (iter = cur; iter < pos; iter++) {
					c = sb.charAt(iter);
					if (c == '(') {
						depth = depth + 1;
					} else if (c == ')') {
