package br.com.company.jcompany.persistence.hibernate.dialect;
...
public class PostgreSQLDialect extends Dialect {
	...
	/*
	 * public boolean dropTemporaryTableAfterUse() { //we have to, because postgres sets current tx //to rollback only after a failed create
	 * table return true; }
	 */
...
final class JDBCExceptionHelper {
	...
	public static String extractSqlState(SQLException sqlException) {
		/*
		 * String sqlState = sqlException.getSQLState(); SQLException nested = sqlException.getNextException(); while ( sqlState == null &&
		 * nested != null ) { sqlState = nested.getSQLState(); nested = nested.getNextException(); } return sqlState;
		 */
	...
	/**
	 * For a given SQLException, locates the most relevant exception based on which has an appropriate error code.
	 * 
	 * @param sqlException
	 *            The exception from which to extract the most relevant exception
	 * @return The most relevant exception, or null if no exception in the chain have an error code.
	 */
	...
	/**
	 * For a given SQLException, locates the most relevant exception based on the X/Open compliant SQLState
	 * 
	 * @param sqlException
	 *            The exception from which to extract the most relevant exception
	 * @return The most relevant exception, or null if no exception in the chain have an SQLState.
	 */
