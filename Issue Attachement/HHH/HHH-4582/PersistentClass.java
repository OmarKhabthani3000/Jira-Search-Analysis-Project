...
public abstract class PersistentClass implements Serializable, Filterable, MetaAttributable {
	...
	protected void checkColumnDuplication(Set distinctColumns, Iterator columns, boolean[] columnUpdateability) 
	throws MappingException {
		int i=0;
		while ( columns.hasNext() ) {
			Selectable columnOrFormula = (Selectable) columns.next();
			if (columnUpdateability[i++]) {
				if (!columnOrFormula.isFormula()) {
					Column col = (Column) columnOrFormula;
					if (!distinctColumns.add(col.getName())) {
						throw new MappingException(
	...
	protected void checkColumnDuplication() {
		...
		if (getIdentifierMapper() == null ) {
			...
			checkColumnDuplication( cols, getKey().getColumnIterator(),getKey().getColumnUpdateability() );
		}
