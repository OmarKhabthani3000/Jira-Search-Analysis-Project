...
public class FieldInterceptionHelper {
	...
	public static boolean isInstrumented(Class entityClass) {
		try {
			if ( Class.forName("net.sf.cglib.transform.impl.InterceptFieldEnabled").isAssignableFrom(entityClass)
			     || Class.forName("org.hibernate.bytecode.javassist.FieldHandled").isAssignableFrom(entityClass) ) {
				return true;
			}
	...
	public static FieldInterceptor extractFieldInterceptor(Object entity) {
		...
		try {
			if ( Class.forName("net.sf.cglib.transform.impl.InterceptFieldEnabled").isAssignableFrom(entity.getClass() ) ) {
				// we have a CGLIB enhanced entity
				return CGLIBHelper.extractFieldInterceptor( entity );
			}
			else if ( Class.forName("org.hibernate.bytecode.javassist.FieldHandled").isAssignableFrom(entity.getClass() ) ) {
