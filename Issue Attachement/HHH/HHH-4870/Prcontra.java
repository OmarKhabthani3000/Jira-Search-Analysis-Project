...
//	private String nif;
	...
	@JoinColumn(name = "SPEXT", referencedColumnName = "SPE_COD")
	@ManyToOne(fetch=FetchType.LAZY)
	private PreSpexterno spext;
	...
	@OneToMany(cascade = CascadeType.ALL, mappedBy = "prcontra",fetch=FetchType.LAZY)
	private Collection<Prcentros> prcentrosCollection;
	@JoinColumns( { @JoinColumn(name = "DOC", referencedColumnName = "DOC",nullable=false,insertable=false,updatable=false),
			@JoinColumn(name = "NUMDOC", referencedColumnName = "NUMDOC",nullable=false,insertable=false,updatable=false) })
	@ManyToOne(fetch=FetchType.LAZY)
	private Docu docu;

	// Ultima factura emitida para ese contrato
	@JoinColumns( { @JoinColumn(name = "ANNOF", referencedColumnName = "ANNO"),
			@JoinColumn(name = "NUMFAC", referencedColumnName = "NUMFAC"),
			@JoinColumn(name = "SERIE", referencedColumnName = "SERIE") })
	@ManyToOne(fetch=FetchType.LAZY)
	private Prfac prfac;
	@JoinColumn(name = "TECNICO", referencedColumnName = "NUSUARIO")
	@ManyToOne(fetch=FetchType.LAZY)
	private Usuarios tecnico;
	@OneToMany(cascade = CascadeType.ALL, mappedBy = "prcontra",fetch=FetchType.LAZY)
	private Collection<Prfac> prfacCollection;
	...
	@Formula(value="( " +
			...
			"    and r.rec_estado>0 " +
			"    and r.rec_tiprec in(0,1,3,10,15,16,17) )")  //Los mismos que en VdsReconocimientoDAO.FILTRO_TIPO_DE_RECONOCIMENTO
...
//	}
	...
	public PreSpexterno getSpext() {
		return spext;
	}

	public void setSpext(PreSpexterno spext) {
		this.spext = spext;
	}
	...
	public Collection<Prcentros> getPrcentrosCollection() {
		return prcentrosCollection;
	}

	public void setPrcentrosCollection(Collection<Prcentros> prcentrosCollection) {
		this.prcentrosCollection = prcentrosCollection;
	}
	...
	public void setDocu(Docu docu) {
		this.docu = docu;
	}

	public Prfac getPrfac() {
		return prfac;
	}

	public void setPrfac(Prfac prfac) {
		this.prfac = prfac;
	}
	...
	public Collection<Prfac> getPrfacCollection() {
		return prfacCollection;
	}

	public void setPrfacCollection(Collection<Prfac> prfacCollection) {
		this.prfacCollection = prfacCollection;
	}
	...
	//START-ENTITY-MOD
	public Empre getEmpresa() {
		return empresa;
	}

	public void setEmpresa(Empre empresa) {
