...
//		put an object in DB
	...
	@Test
	public void testUpgradeLockBug() throws Exception {
		Session s = openSession();
		Transaction tx = s.beginTransaction();
		Flight airFrance = new Flight();
		Long id = new Long( 747 );
		airFrance.setId( id );
		airFrance.setName( "Paris-Amsterdam" );
		airFrance.setDuration( new Long( 10 ) );
		airFrance.setDepartureDate( new Date( 05, 06, 21, 10, 0, 0 ) );
		...
		// Load an entity using HQL in read-only mode
		airFrance = (Flight) s.createQuery( "from Flight where id = 747" ).setReadOnly( false ).uniqueResult();
		assertNotNull( airFrance );
		System.err.println(s.contains( airFrance ));
		System.err.println(s.getCurrentLockMode( airFrance ));
		...
		// Load the same entity using get() with an UPGRADE lock
		airFrance = (Flight) s.load( Flight.class, id, LockOptions.UPGRADE );
		assertNotNull( airFrance );
		assertEquals( id, airFrance.getId() );
