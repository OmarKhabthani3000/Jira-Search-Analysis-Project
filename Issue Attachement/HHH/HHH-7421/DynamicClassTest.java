...
public class DynamicClassTest extends BaseCoreFunctionalTestCase {
  ...
  /**
   * A test case for https://hibernate.onjira.com/browse/HHH-7421
   */
	@Test
	public void testUpgradeLockBug() {
		Session s = openSession();
		Transaction t = s.beginTransaction();
    ...
    // Load an entity using HQL in read-only mode		
		cars = (Map) s.createQuery("from ProductLine pl order by pl.description").setReadOnly(true).uniqueResult();
		String id = (String) cars.get("id");
		assertNotNull(cars);
		
		// Load the same entity using get() with an UPGRADE lock
		cars = (Map) s.get("ProductLine", id, LockOptions.UPGRADE);
		assertNotNull(cars);
		assertEquals(id, (String) cars.get("id"));
	}
